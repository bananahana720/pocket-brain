...
function safeParseJson<T>(value: string): T {
  ...
  const parseOrThrow = (candidate: string): T => {
  ...
  try {
    ...
    if (fencedMatch?.[1]) {
      const parsed = parseCandidate(fencedMatch[1].trim());
      if (parsed !== null) {
        return parsed;
      }
    }
    ...
    if (firstArray && firstObject) {
      const arrayIndex = firstArray.index ?? Number.MAX_SAFE_INTEGER;
      const objectIndex = firstObject.index ?? Number.MAX_SAFE_INTEGER;
      if (arrayIndex <= objectIndex) {
        return parseOrThrow(firstArray[0]);
      }
      return parseOrThrow(firstObject[0]);
    }
...
function extractBatchItems(parsed: any): any[] {
...
): Array<{ content: string; title: string; tags: string[]; type: BatchItemType }> {
  const limited = rawItems.slice(0, MAX_BATCH_ITEMS);
  const normalized = limited.map((item: any) => {
    const itemType = item?.type === 'TASK' || item?.type === 'IDEA' || item?.type === 'NOTE' ? item.type : null;
    const type = hints.generationLikely && hints.requestedType ? hints.requestedType : itemType || 'NOTE';
    ...
    try {
      ...
      if (pathname.startsWith('/api/v1/ai/') && request.method === 'POST') {
        ...
        if (pathname === '/api/v1/ai/batch') {
          ...
          const prompt = buildBatchPrompt(content, batchHints);
          const text = await withRetries(() =>
            callProvider(env, credentials.provider, credentials.apiKey, prompt, env.DEFAULT_MODEL)
          );
          const parsed = safeParseJson<any>(text);
          const items = extractBatchItems(parsed);
          const results = normalizeBatchResults(items, content, batchHints);
